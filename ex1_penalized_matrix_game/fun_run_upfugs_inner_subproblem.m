function trial = fun_run_upfugs_inner_subproblem(A, grad_g_outer, state, alg)
% fun_run_upfugs_inner_subproblem
% Run the PFUGS inner line-search subroutine for one outer trial.
    ...
    for t = 1:alg.max_inner_it
        ...
        terminate_now = false;
        if t == 1
            % terminate_now = abs(M_t - L_trial) <= alg.ineq_tol * max(1, L_trial);
            terminate_now = abs(M_t - L_trial) <= alg.ineq_tol;  
        % elseif used_special_c_prev && abs(M_t - M_base) <= alg.ineq_tol * max(1, M_base)
        ...
        if M_t * alpha_tilde^2 <= L_trial + alg.ineq_tol % * max(1, L_trial)
    ...
    error('PFUGS inner subproblem reached max_inner_it without termination.');
end
