function res = fun_run_upfugs_matrixgame(A, B, d, lambda, alg, phi_star, x0)
% fun_run_pfugs_matrixgame
% ------------------------------------------------------------
% Run the parameter-free universal gradient sliding method for
%   Phi(x) = f(x) + g(x)
% where
%   f(x) = max_j <A_j, x>      (inner nonsmooth part)
%   g(x) = lambda * ||B*x-d||^2 (outer smooth part).
    ...
    if alg.verbose
        if has_phi_star
            fprintf('PFUGS iter %5d | gap = %.2e\n', 0, gap0);
        else
            fprintf('PFUGS iter %5d | phi(bar_x_k) = %.2e\n', 0, phi0);
        end
    end
    ...
    for k = 1:alg.maxit
        if k == 1
            [trial_accept, L_accept, trial_count, Gamma_k, oracle_k] = ...
                fun_run_upfugs_first_outer_iteration(A, B, d, lambda, x_prev, xtil_prev, ...
                xbar_prev, alg);
        else
            [trial_accept, L_accept, trial_count, Gamma_k, oracle_k] = ...
                fun_run_upfugs_standard_outer_iteration(A, B, d, lambda, x_prev, xtil_prev, ...
        ...
        phi_bar_k = fun_eval_objective(A, B, d, lambda, xbar_prev);
        if has_phi_star
        ...
        if has_phi_star
            ...
            if should_print
                fprintf(['PFUGS iter %5d | gap = %.2e | L_k = %.2e | ' ...
                    'outer_trials = %d | accepted_T = %d | total_inner_iters = %d\n'], ...
                    k, gap_k, L_accept, ...
                    trial_count, trial_accept.accepted_inner_iters, trial_accept.total_inner_iters);
            end

            if gap_k <= alg.epsilon + alg.ineq_tol
                stop_reason = 'epsilon';
                break;
            end
        else
            should_print = alg.verbose && (k <= print_first ...
                ...
                || k == alg.maxit);
            if should_print
                fprintf(['PFUGS iter %5d | phi(bar_x_k) = %.2e | L_k = %.2e | ' ...
                    'outer_trials = %d | accepted_T = %d | total_inner_iters = %d\n'], ...
                    k, phi_bar_k, L_accept, ...
                    trial_count, trial_accept.accepted_inner_iters, trial_accept.total_inner_iters);
            end
        end
    end
