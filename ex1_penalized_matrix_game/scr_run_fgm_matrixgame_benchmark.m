...
if report.run_benchmark
    aggregate_rows = cell(num_eps, 9);
else
    aggregate_rows = cell(num_eps, 5);
end
...
for eps_idx = 1:num_eps
    ...
    if report.run_benchmark
        ...
        summary_rows = zeros(data_params.num_repeats, 5);
    end
        ...
        if report.run_benchmark
            ...
            if repeat_idx == 1
                ...
                fprintf('%-10s %-12s %-12s %-12s %-12s\n', ...
                    'seed', 'PFUGS time', 'PFUGS gap', 'PFUGS fsubg', 'PFUGS ggrad');
            end
            fprintf('%-10d %-12.2f %-12.2e %-12.2f %-12.2f\n', ...
                params.seed, result_pfugs.time_sec, result_pfugs.gap, ...
                result_pfugs.f_subgrad, result_pfugs.g_grad);
        end
    ...
    if report.run_benchmark
        ...
        fprintf('%-10s %-12s %-12s %-12s %-12s\n', ...
            'seed', 'PFUGS time', 'PFUGS gap', 'PFUGS fsubg', 'PFUGS ggrad');
        for repeat_idx = 1:data_params.num_repeats
            fprintf('%-10.0f %-12.2f %-12.2e %-12.2f %-12.2f\n', ...
                summary_rows(repeat_idx, 1), summary_rows(repeat_idx, 2), ...
                summary_rows(repeat_idx, 3), summary_rows(repeat_idx, 4), ...
                summary_rows(repeat_idx, 5));
        end
        fprintf('%-10s %-12s %-12s %-12s %-12s\n', ...
            'mean(std)', ...
            sprintf('%.2f(%.2f)', metric_means(1), metric_stds(1)), ...
...
if report.run_benchmark
    ...
    fprintf('%-10s %-16s %-18s %-16s %-16s\n', ...
        'epsilon', 'PFUGS time', 'PFUGS gap', 'PFUGS fsubg', 'PFUGS ggrad');
end
for eps_idx = 1:num_eps
    if report.run_benchmark
        fprintf('%-10s %-16s %-18s %-16s %-16s %-16s %-18s %-16s %-16s\n', ...
        ...
        fprintf('%-10s %-16s %-18s %-16s %-16s\n', ...
            aggregate_rows{eps_idx, 1}, aggregate_rows{eps_idx, 2}, ...
            aggregate_rows{eps_idx, 3}, aggregate_rows{eps_idx, 4}, ...
            aggregate_rows{eps_idx, 5});
    end
end

if report.save_latex_table
    latex_table_path = fullfile(report.output_dir, report.latex_table_name);
    fun_write_experiment1_aggregate_table_tex(aggregate_rows, latex_table_path, report.run_benchmark);
